 ...
 *   - User 1: creates a room, then picks a challenge from the room lobby
 *   - User 2: joins the room using the invite code returned by the API (challenge already set)
 *
 * The invite code is captured via page.waitForResponse() on POST /api/rooms
 * because the UI redirects to the room lobby without displaying the code after creation.
 *
...
test.describe('Room collaboration — 2 browser contexts', () => {
  ...
  test('user 1 creates room, user 2 joins with invite code', async ({ browser }: { browser: Browser }) => {
    ...
    // Navigate to /colaboracion and create a room (no challenge picked yet)
    await page1.goto('/colaboracion');
    await expect(page1.getByRole('button', { name: 'Crear sala' })).toBeVisible({ timeout: 10_000 });
    ...
    const [createResponse] = await Promise.all([
      ...
      page1.getByRole('button', { name: 'Crear sala' }).click(),
    ]);
    ...
    // User 1 lands in the room lobby (no challenge assigned yet) and picks one
    await page1.waitForURL(/\/sala\/.+/, { timeout: 15_000 });
    await expect(page1.locator('.challenge-card').first()).toBeVisible({ timeout: 15_000 });
    await page1.locator('.challenge-card').first().click();
